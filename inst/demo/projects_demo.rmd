...
optim_proj=projects_optimweightsandcurves(ps, method=c('SANN'), trace=1, reltol=1)
# res
...
res=projects_optimweightsandcurves(ps2, method=c('Nelder-Mead'), trace=1, doparallel=F)
...
o=list_merge(list(
    ...
    ,logbestfile='' #Excel file to log best results. if non-empty, we'll log the final best values.
  ),o,list());

if(o$maximize && (is.null(o$control)||is.null(o$control$fnscale))){ o$control$fnscale=-1; }

myoptim(par,projects_evalweightandcurvevector, o, ps=ps, weights=weights,Iweights=Iweights, enforceweightsign=o$enforceweightsign, curves=curves,Icurves=Icurves,stopwhenbestpossibleisfound=T)
