...
for(folder in csv('sfs.forward,sfs.reverse.withoptim,sfs.reverse.withoutoptim')){
  ...
  g=ggplot(dg, aes(x=datasource,y=impact,fill=datasource)) + geom_bar(stat = 'identity',color='black') + ggtitle(folder) + scale_fill_brewer(palette = 'PRGn') + theme_classic() + ylab("MAP Score Impact") + theme(axis.text.x=element_text(angle = 90, hjust=0.95,vjust=0.2)) + theme(text = element_text(size=14));
  print(g);
}
